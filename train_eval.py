...
def train_eval(
  ...
  train_dir = os.path.join(root_dir, 'dqn', 'train')
  eval_dir = os.path.join(root_dir, 'dqn', 'eval')
  image_dir = os.path.join(root_dir, 'dqn', 'images')
  font_path = os.path.join(root_dir, 'resources', 'Roboto-Regular.ttf')
  ...
  with tf.compat.v2.summary.record_if(
    ...
    train_py_env = Environment.puzzleEnv(tile_set=tile_set)
    eval_py_env = Environment.puzzleEnv(tile_set=tile_set)
    ...
    def create_policy_eval_video(policy, num_episodes=5):
      frames = []
      image_path = os.path.join(image_dir, 'environment_{}.png'.format(global_step.numpy()))
      ...
      for episode in range(num_episodes):
        ...
        while not time_step.is_last():
          ...
          frames.append(img)
      frames[0].save(image_path, save_all=True, append_images=frames[1:], optimize=True, duration=len(frames)/2, loop=0, format="PNG")
    ...
    for _ in range(num_iterations):
      ...
      if global_step.numpy() % train_checkpoint_interval == 0:
        ...
        create_policy_eval_video(eval_policy)
